...
class AvroSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("test reading/writing primitive and complex types") {
    TestUtils.withTempDir { tempDir =>
      // Read avro file saved on the last step
      ...
      val arrayDF = spark.read.avro("src/test/resources/randomStringArray.avro")
      assert(arrayDF.count == 20)
      assert(arrayDF.rdd.count == 20)
      arrayDF.write.avro(tempDir + "/randomStringArray.avro")
      assert(spark.read.avro(tempDir + "/randomStringArray.avro").count == 20)
      
      val mapDF = spark.read.avro("src/test/resources/randomLongMap.avro")
      assert(mapDF.count == 20)
      assert(mapDF.rdd.count == 20)
      mapDF.write.avro(tempDir + "/randomLongMap.avro")
      assert(spark.read.avro(tempDir + "/randomLongMap.avro").count == 20)
    }
  }
}
